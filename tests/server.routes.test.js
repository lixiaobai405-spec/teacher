  ...
  await assert.rejects(
    () => client.complete({
      ...
      diagnoseDetails: () => ({
        ...
        numberLocations: [
          { field: 'frequency', kind: 'arabic:分钟' },
          { field: 'entry[0]', kind: 'chinese:个' },
          { field: 'private employee text', kind: 'arabic:分钟' },
          { field: 'scripts[0]', kind: 'invalid:15分钟' },
        ],
      }),
      buildRetryMessage: (issues) => `PLAN_CONTRACT_REPAIR\n${issues.join('\n')}`,
    }),
...
test('步骤 3 允许明确建议的次数与时长且数字纠错不增加重试', async () => {
  ...
  assert.match(step3Prompt, /明确.*建议|未来行动/);
  assert.match(step3Prompt, /次数|时长/);
  assert.match(step3Prompt, /员工现状|既成结果|事实依据/);
  assert.match(step3Prompt, /Goal（目标）.*Options（可选方案）.*Will（行动承诺）.*次数.*时长/s);
  assert.match(step3Prompt, /一个动作.*明确动作/);
  ...
  assert.equal(capturedOptions.validate(planResult({
    frequency: '建议每周沟通1次，每次15分钟。',
  })), true);

  assert.deepEqual(capturedOptions.diagnoseDetails(planResult({
    entry: ['建议先明确一个目标。'],
  })), {
    numberKinds: ['chinese:个'],
    numberLocations: [
      { field: 'entry[0]', kind: 'chinese:个' },
    ],
  });
  ...
  assert.match(correction, /明确.*建议|未来行动/);
  assert.match(correction, /次数|时长/);
  assert.match(correction, /员工现状|既成结果|事实依据/);
  assert.match(correction, /Goal（目标）.*Options（可选方案）.*Will（行动承诺）.*次数.*时长/s);
  assert.match(correction, /一个动作.*明确动作/);
