...
function acceptNextUnsavedConfirmation(page) {
...
test('类型判定期间显示匹配画像文案且返回上一步会取消加载', async ({ page }) => {
  ...
  acceptNextUnsavedConfirmation(page);
...
test('第 1 步上一步确认后返回登录后首页且保持登录', async ({ page }) => {
  await page.goto('/');
  await openIntake(page);
  ...
  await page.getByRole('button', { name: '返回上一步' }).click();
  expect(confirmationMessage).toBe('当前内容尚未保存，离开后将丢失。是否继续？');

  await expect(page.getByRole('heading', { name: '因材施教，给每个人对的辅导方式' }))
    .toBeVisible();
  await expect(page.getByRole('button', { name: '退出登录' })).toBeVisible();
  await expect(page.locator('#auth-user')).toContainText('Frontend_Test_User');
});

test('方案尚未生成时上一步和返回首页都要求未保存确认', async ({ page }) => {
  await page.goto('/');
  await openIntake(page);
  await page.getByLabel('绩效目标 / 上层期望').fill('尚未保存的目标');
  const confirmations = [];
  ...
  expect(confirmations).toEqual([
    '当前内容尚未保存，离开后将丢失。是否继续？',
    '当前内容尚未保存，离开后将丢失。是否继续？',
  ]);
});

test('历史同步失败时上一步和返回首页都要求未保存确认', async ({ page }) => {
