...
- [x] **Step 1: 写“未变化复用”和“变化后清理”的失败测试**
...
- [x] **Step 4: 在 intake 与 classify 成功边界使用快照**
...
- [x] **Step 5: 让已有方案前进不调用 API，并在新方案成功后清除反馈**
...
- [x] **Step 3: 在 `goPrevious()` 接入现有请求取消机制**
...
- [x] 第 2、3、4 步均有“返回上一步”，第 1 步没有。
- [x] 返回只移动到相邻前一步，不返回首页、不调用 API、不立即清除数据。
- [x] 返回后不修改数据，可以无 API 地继续查看旧分类、旧方案和旧反馈。
- [x] 员工信息或追问答案实际变化后，旧信息审查、分类、方案和反馈按依赖关系清除。
- [x] 新分类输入实际变化后，旧方案和反馈清除。
- [x] 新方案成功生成后，旧反馈输入和反馈结果清除；生成失败时保留旧内容。
- [x] 返回操作取消当前请求，迟到成功或失败响应均无法覆盖当前页面。
- [x] 320px 和 375px 下按钮可见且无横向溢出。
- [x] 不新增后端接口、持久化存储、跨会话记忆或外部系统集成。
- [x] `npm.cmd test` 完整通过，且未调用真实模型。
