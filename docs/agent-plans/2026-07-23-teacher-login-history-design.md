...
10. 第 3 步去除重复展示：`绩效差距修正方法`保留 SBI，`话术示例`只展示 GROW。
...
- `绩效差距修正方法`展示 SBI：Situation（情境）、Behavior（行为）、Impact（影响）。
- `话术示例`只展示 GROW：Goal（目标）、Reality（现状）、Options（可选方案）、Will（行动承诺）。

该调整只作用于前端展示层。服务端模型响应、`plan.scripts` 数据、模型提示词和现有业务契约保持不变；前端在渲染 `#plan-scripts` 时按阶段标签过滤重复的 SBI 内容，`#plan-gap-fix` 仍完整渲染 SBI。历史记录继续保存原始且通过契约校验的方案快照，避免展示规则改变持久化数据。
...
- 第 3 步 `话术示例`只显示 GROW，`绩效差距修正方法`仍显示完整 SBI。
- 未保存离开确认、请求取消和迟到响应隔离。
...
- [ ] 第 3 步 `话术示例`不再重复显示 SBI，`绩效差距修正方法`中的 SBI 保持可见。
- [ ] SBI 去重只影响前端展示，不修改提示词、模型响应契约或历史快照。
- [ ] 未保存离开有二次确认，进行中请求会取消。
