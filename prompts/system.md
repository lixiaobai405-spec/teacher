...
- frequency:建议沟通频率(采用该格建议频率；可将次数或时长写成明确的未来建议，也可使用低频、中频、高频、持续跟进、按项目节点等定性节奏)
...
步骤 3 的有限例外：frequency、Goal（目标）、Options（可选方案）、Will（行动承诺），以及明确标注为“建议”“可以”“可考虑”“下一步”“后续”“未来”或“计划”的未来行动，可以新增次数或时长。Reality（现状）及 Situation（情境）、Behavior（行为）、Impact（影响）等事实段不得新增数字。所有新增数字不得写成员工现状、既成结果或事实依据；比例、人数、金额、分数、事项数量和对象数量仍必须有事实来源。不得写“一个动作”“两个方案”等无来源对象数量，改为“明确动作”“比较可选方案”等无数字表达。首次生成和重新生成遵守相同边界。
